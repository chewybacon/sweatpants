...
export function toolDiscoveryPlugin(
  ...
  return {
    name: '@sweatpants/framework:tool-discovery',
...
async function generateRegistry(
  ...
  const agentCoreOutFile = join(root, options.agentCoreOutFile)
  ...
  try {
    ...
    if (options.generateAgentCoreRegistry) {
      await writeAgentCoreRegistryFile(agentCoreOutFile, [], options)
    }
    return
  }
  ...
  if (options.generateAgentCoreRegistry) {
    await writeAgentCoreRegistryFile(agentCoreOutFile, tools, options)
    log(options, 'normal', `[tool-discovery] Generated ${relative(root, agentCoreOutFile)}`)
  }
}
...
async function writeAgentCoreRegistryFile(
  outFile: string,
  tools: DiscoveredTool[],
  options: ResolvedToolDiscoveryOptions
): Promise<void> {
  ...
  const content = generateAgentCoreRegistryContent(tools, outFile, options)
  await writeFile(outFile, content)
}
 ...
 */
export function generateAgentCoreRegistryContent(
  tools: DiscoveredTool[],
  outFile: string,
  options: ResolvedToolDiscoveryOptions
): string {
  ...
  const supportedNames = new Set(options.agentCoreSupportedToolNames)
  const supportedTools = tools.filter((tool) => {
    const canonicalName = tool.originalToolName ?? tool.toolName
    return supportedNames.has(canonicalName) || supportedNames.has(tool.toolName)
  })

  const sortedTools = [...supportedTools].sort((a, b) => {
    const aCanonicalName = a.originalToolName ?? a.toolName
    const bCanonicalName = b.originalToolName ?? b.toolName
    return aCanonicalName.localeCompare(bCanonicalName) || a.toolName.localeCompare(b.toolName)
  })
  ...
  for (const tool of sortedTools) {
    ...
    const canonicalName = tool.originalToolName ?? tool.toolName
    const aliases = tool.toolName === canonicalName ? [] : [tool.toolName]
    ...
    aliasEntries.push(`  ['${canonicalName}', '${canonicalName}'],`)
    for (const alias of aliases) {
      aliasEntries.push(`  ['${alias}', '${canonicalName}'],`)
    }
  }
 ...
 *
 * DO NOT EDIT - This file is generated by @sweatpants/framework
 * Regenerate by modifying files in ${options.dir}/ or AgentCore support config.
 *
