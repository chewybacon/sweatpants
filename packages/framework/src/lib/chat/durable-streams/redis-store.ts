...
interface RedisDurableState<T> {
  tokens: T[]
  completed: boolean
  errorMessage: string | null
}

export interface RedisStoreAdapter {
  get(key: string): Promise<string | null>
  set(key: string, value: string): Promise<void>
  del(key: string): Promise<void>
  waitForChange?(channel: string, afterLSN: number): Promise<void>
  publishChange?(channel: string): Promise<void>
}

export interface RedisStoreConfig<T> {
...
function* readBufferState<T>(
  ...
  key: string,
  deserialize: (token: string) => T,
): Operation<RedisDurableState<T> | null> {
  const raw = yield* call(() => adapter.get(key))
  if (!raw) {
    return null
  }
  const parsed = fromJson<RedisDurableState<string>>(raw)
  return {
...
function* writeBufferState<T>(
  adapter: RedisStoreAdapter,
  key: string,
  state: RedisDurableState<T>,
  serialize: (token: T) => string,
): Operation<void> {
  const persisted: RedisDurableState<string> = {
...
export function createRedisBufferStore<T>(
  config: RedisStoreConfig<T>,
): TokenBufferStore<T> {
  const {
    adapter,
    serialize,
  ...
  const createBuffer = (id: string): TokenBuffer<T> => {
    ...
    return {
      ...
      *append(tokens: T[]): Operation<number> {
        ...
        if (adapter.publishChange) {
          yield* call(() => adapter.publishChange!(channel))
        }
  ...
  return {
    *create(id: string): Operation<TokenBuffer<T>> {
      ...
      const initial: RedisDurableState<T> = {
        tokens: [],
        completed: false,
        errorMessage: null,
      }
...
export function createRedisRegistryStore(
  adapter: RedisStoreAdapter,
  keyPrefix = 'durable-streams',
): SessionRegistryStore {
  return {
