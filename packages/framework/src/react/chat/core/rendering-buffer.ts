...
export interface RenderingBufferOptions {
  /**
...
/**
 * Create a rendering buffer transform.
 *
 ...
 */
export function renderingBufferTransform(
  options: RenderingBufferOptions = {}
): PatchTransform {
  const {
  ...
  return function* (
    ...
    const log = debug
      ? (msg: string, data?: unknown) =>
          console.log(`[renderingBuffer] ${msg}`, data ?? '')
      : () => {}
        ...
        return {
          *[Symbol.iterator]() {
            ...
            if (processed['deltaHtml'] || processed['chunkHtml']) {
              newRenderable.deltaHtml = (processed['deltaHtml'] ||
                processed['chunkHtml']) as string
            }
            if (processed['meta']) {
            ...
            const revealHint = processed['revealHint'] as
              | BufferRenderablePatch['revealHint']
              | undefined
    ...
    function* processAll(): Operation<void> {
      ...
      for (const result of results) {
        ...
        if (!raw.startsWith(content)) {
          console.warn(
            `[renderingBuffer] settler yielded content that's not a prefix of raw. ` +
