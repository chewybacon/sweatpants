...
describe('readNdjson', () => {
  describe('basic parsing', () => {
    it('should parse single JSON line', async () => {
      ...
      const results = await collectAll(readNdjson<{ type: string; content: string }>(stream))
    ...
    it('should parse multiple JSON lines', async () => {
      ...
      const results = await collectAll(readNdjson<{ id: number }>(stream))
    ...
    it('should handle empty lines between JSON objects', async () => {
      ...
      const results = await collectAll(readNdjson<object>(stream))
  ...
  describe('complex JSON values', () => {
    ...
    it('should handle strings with escaped newlines', async () => {
      ...
      const results = await collectAll(readNdjson<{ text: string }>(stream))
    ...
    it('should handle unicode in strings', async () => {
      ...
      const results = await collectAll(readNdjson<{ emoji: string }>(stream))
  ...
  describe('abort signal', () => {
    it('should stop reading when abort signal is triggered before start', async () => {
      ...
      const stream = createMockStream(['{"a":1}\n{"b":2}\n'])
      const results = await collectAll(readNdjson(stream, { signal: controller.signal }))
    ...
    it('should stop reading when abort signal is triggered mid-stream', async () => {
      ...
      const results = await collectAll(readNdjson<{ id: number }>(stream, { signal: controller.signal }))
  ...
  describe('error handling', () => {
    it('should throw on malformed JSON', async () => {
      ...
      await expect(collectAll(readNdjson(stream))).rejects.toThrow()
    })
  ...
  describe('empty stream', () => {
    it('should handle empty stream', async () => {
      const stream = createMockStream([])
      const results = await collectAll(readNdjson(stream))
  ...
  describe('stream event types (real-world scenarios)', () => {
    it('should parse typical chat streaming events', async () => {
      ...
      const results = await collectAll(readNdjson<{ type: string }>(stream))
    ...
    it('should handle tool call events', async () => {
      ...
      const results = await collectAll(readNdjson<ToolEvent>(stream))
