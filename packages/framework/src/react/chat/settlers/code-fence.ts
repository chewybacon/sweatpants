/**
 * settlers/code-fence.ts
 *
 * Code fence-aware settler for progressive syntax highlighting.
 *
 * This settler understands markdown code fences and yields content
 ...
 */
import type { SettleContext, SettleResult, MetadataSettler, BaseSettleMeta } from '../types/settler'
...
/**
 * Metadata for code fence settler.
 *
 * This extends BaseSettleMeta with code-fence-specific fields.
 */
export interface CodeFenceMeta extends BaseSettleMeta {
 ...
 * - Tracks fence delimiter length (for nested code blocks in markdown)
 * - Handles flush mode for stream end (OpenAI behavior - fence close without \n)
 *
 * @example
 * ```typescript
 * renderingBufferTransform({
 *   settler: codeFence,
 *   processors: [shikiPlugin.processor],
 * })
 * ```
 */
export function codeFence(): MetadataSettler<CodeFenceMeta> {
  ...
  return function* (ctx: SettleContext): Iterable<SettleResult<CodeFenceMeta>> {
    ...
    while (position < remaining.length) {
        ...
        if (flush) {
          ...
          if (inFence) {
            ...
            const isClosingFence =
              FENCE_END_REGEX.test(trimmed) &&
              fenceDelimiter.length > 0 &&
      ...
      if (!inFence) {
        ...
        const isClosingFence =
          ...
          endMatch[1] &&
          fenceDelimiter.length > 0 &&
          endMatch[1].startsWith(fenceDelimiter[0]!) &&
          endMatch[1].length >= fenceDelimiter.length
