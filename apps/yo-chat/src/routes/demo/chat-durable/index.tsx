/**
 * /demo/chat-durable - Durable Chat Demo
 *
 * Demonstrates the durable chat handler with:
 * - Session reconnection from last LSN
 * - Multi-client fan-out (multiple tabs can share a session)
 * - Full session replay
 *
 * Uses the same UI as /demo/chat but points to /api/chat-durable.
 * Once feature parity is confirmed, this will replace the regular chat demo.
 */
...
export const Route = createFileRoute('/demo/chat-durable/')({
  component: DurableChatDemo,
})
...
function DurableChatDemo() {
  // High-level useChat hook pointing to the DURABLE endpoint
  const {
    messages,
    streamingMessage,
    isStreaming,
    ...
    // Use the durable chat API endpoint
    baseUrl: '/api/chat-durable',
    // Enable durable format (unwraps { lsn, event } wrapper)
    durable: true,
    // Use the full pipeline (markdown + shiki + mermaid)
    pipeline: 'full',
    // Enable tools
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 flex items-end justify-between border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Using /api/chat-durable with session reconnection support
            </p>
          </div>
        ...
        <div className="mt-4 flex justify-between text-xs text-slate-600 px-2">
          ...
          <div className="flex items-center gap-4">
            <span>
              <span className="text-amber-500">durable</span>
              {' mode'}
            </span>
